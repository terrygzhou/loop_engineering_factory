"""
REVIEW node: Human-in-the-loop architecture review gate.
...
def review_node(state: dict) -> dict:
    """
    REVIEW phase: Human architecture review gate between PLAN and BUILD.
    ...
    if auto_approve:
        ...
        audit.log_node_output("REVIEW", {"approved": True, "reason": "auto_approve"})
        return state
    ...
    if approved:
        state["diagram_status"] = "approved"
        state["next_phase"] = "BUILD"
        print("  ✓ REVIEW approved — proceeding to BUILD")
        audit.log_node_output("REVIEW", {"approved": True, "comments": ""})
        audit.log_node_transition("REVIEW", "BUILD", "plan approved")
    else:
