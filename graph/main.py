...
def build_graph(checkpointer=None, auto_approve=False):
    ...
    # DISCOVER uses in-node interrupt() for two sequential HIL pauses.
    # Adding it to interrupt_after would suppress in-node interrupt() calls
    # on resume — so it stays out, and the bridge catches __interrupt__ chunks.
    hil_nodes = [
        "DISCOVER",
        "ARCH_REVIEW",
        "REFLECT",
