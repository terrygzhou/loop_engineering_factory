...
def _resolve(
    env_var: str | None, config: Dict[str, Any], key_path: str, default: str
) -> str:
...
class Config:
    ...
    class Paths:
        project_name: str = _resolve(
            None, _config, "paths.project_name", "loop_project"
        )
        project_path_template: str = _resolve(
            None, _config, "paths.project_path", "{{project_name}}"
        )
        skills_dir: str = _resolve("SKILLS_DIR", _config, "paths.skills_dir", "skills")
        storage_dir: str = _resolve(
            "STORAGE_DIR", _config, "paths.storage_dir", "./storage"
        )
        guardrails_path: str = _resolve(
            "GUARDRAILS_PATH",
            _config,
            "paths.guardrails_path",
            "./config/guardrails.yaml",
        )
        build_dir: str = _resolve("BUILD_DIR", _config, "paths.build_dir", "/app/build")
        workspace_dir: str = _resolve(
            "WORKSPACE_DIR", _config, "paths.workspace_dir", "./output"
        )
        output_subdir: str = _resolve(
            "OUTPUT_SUBDIR", _config, "paths.output_subdir", "output"
        )
        output_dir: str = _resolve(
            "OUTPUT_DIR", _config, "paths.output_dir", "/app/output"
        )
        prompt_log_dir: str = _resolve(
            "PROMPT_LOG_DIR", _config, "paths.prompt_log_dir", "build/prompt_logs"
        )
        ...
        def project_path(self) -> str:
            ...
            if "{{project_name}}" in template:
                return os.path.join(
                    os.path.expanduser(self.workspace_dir), self.project_name
                )
            return template

    class Services:
        class LLM:
            base_url: str = _resolve(
                "LLM_BASE_URL",
                _config,
                "services.llm.base_url",
                "http://pop-os:8080/v1",
            )
            model: str = _resolve(
            ...
            temperature: float = float(
                _resolve("LLM_TEMPERATURE", _config, "services.llm.temperature", "0.1")
            )
            max_tokens: int = int(
                _resolve("LLM_MAX_TOKENS", _config, "services.llm.max_tokens", "32768")
            )

        class Chroma:
            url: str = _resolve(
                "CHROMA_URL", _config, "services.chroma.url", "http://chromadb:8000"
            )
            port: int = int(_resolve(None, _config, "services.chroma.port", "8000"))

        class OTEL:
            endpoint: str = _resolve(
                "OTEL_EXPORTER_OTLP_ENDPOINT",
                _config,
                "services.otel.endpoint",
                "http://otel-collector:4318/v1/traces",
            )
            service_name: str = _resolve(
                "OTEL_SERVICE_NAME",
                _config,
                "services.otel.service_name",
                "loop-orchestrator",
            )
            exporter_port: int = int(
                _resolve(None, _config, "services.otel.exporter_port", "4318")
            )
        ...
        class Phoenix:
            ...
            database_uri: str = _resolve(
                None,
                _config,
                "services.phoenix.database_uri",
                "/var/lib/phoenix/phoenix.db",
            )

        class Postgres:
            user: str = _resolve(
                "POSTGRES_USER", _config, "services.postgres.user", "postgres"
            )
            password: str = _resolve(
                "POSTGRES_PASSWORD", _config, "services.postgres.password", "postgres"
            )
            database: str = _resolve(
                "POSTGRES_DB", _config, "services.postgres.database", "crm_dev"
            )
            port: int = int(_resolve(None, _config, "services.postgres.port", "5432"))
            host: str = _resolve(
                "POSTGRES_HOST", _config, "services.postgres.host", "localhost"
            )
        ...
        class LoopAPI:
            url: str = _resolve(
                "LOOP_API_URL",
                _config,
                "services.loop_api.url",
                "http://localhost:48011",
            )
            port: int = int(_resolve(None, _config, "services.loop_api.port", "48011"))

        class Product:
            url: str = _resolve(
                "PRODUCT_URL", _config, "services.product.url", "http://localhost:8010"
            )
            port: int = int(_resolve(None, _config, "services.product.port", "8010"))

        class Observability:
            port: int = int(
                _resolve("OBSERVABILITY_PORT", _config, "observability.port", "8081")
            )

        class OpenHands:
            url: str = _resolve(
                "OH_URL", _config, "openhands.url", "http://openhands:8000"
            )
            secret_key: str = _resolve(
                "OH_SECRET_KEY", _config, "openhands.secret_key", "changeme"
            )
            workspace_path: str = _resolve(
                "OH_WORKSPACE",
                _config,
                "openhands.workspace_path",
                "/opt/workspace_base/output",
            )
            timeout: int = int(
                _resolve("OH_TIMEOUT", _config, "openhands.timeout", "3600")
            )
            poll_interval: int = int(
                _resolve(None, _config, "openhands.poll_interval", "5")
            )
            prompt_char_limit: int = int(
                _resolve(None, _config, "openhands.prompt_char_limit", "16000")
            )
            profile_name: str = _resolve(
                None, _config, "openhands.profile_name", "build_agent"
            )
    ...
    class Observability:
        prometheus_port: int = int(
            _resolve(None, _config, "observability.prometheus_port", "9090")
        )
        loki_port: int = int(_resolve(None, _config, "observability.loki_port", "3100"))
        port: int = int(
            _resolve("OBSERVABILITY_PORT", _config, "observability.port", "8081")
        )
        log_level: str = _resolve(
            "LOG_LEVEL", _config, "observability.log_level", "INFO"
        )
    ...
    class Workflow:
        ...
        auto_approve: bool = _resolve(
            "AUTO_APPROVE", _config, "workflow.auto_approve", "false"
        ).lower() in ("true", "1", "yes")
        skill_registry_path: str = _resolve(
    ...
    def set_project_name(self, name: str):
        ...
        _config["paths"] = {
            ...
            "storage_dir": self.paths.storage_dir,
        ...
        try:
            ...
            pass
        print(
            f"  ✓ config.yaml updated: project_name={name}, project_path={self.paths.project_path}"
        )
